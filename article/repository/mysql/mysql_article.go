...
import (
	...
	"github.com/NASA-Look-For-DUD/article/repository"
...
func (m *mysqlArticleRepository) GetFireLocation(ctx context.Context, ar *domain.Article) (res []domain.Article, err error) {
	query := `SELECT id, title, address, content, author_id, 
		latitude, longitude, updated_at, created_at   
			FROM fire_location WHERE created_at = ?`

	list, err := m.fetch(ctx, query, time.Now())
	if err != nil {
		return
	}
